...
Scenario
--------
FastBox runs several warehouses and delivery agents. Every day a batch of
packages needs to be picked up from a warehouse and dropped at a
destination. This script simulates one day of operations and reports:

    * how many packages each agent delivered
    * the total distance each agent travelled
    * an "efficiency" score for each agent
    * which agent was the most efficient overall

How to run
----------
    python delivery_system.py data.json [report.json]

If no arguments are given it looks for "data.json" in the current folder
and writes "report.json" next to it.

Design notes
------------
The task input has been supplied in two slightly different JSON shapes
across the sample files (a "base case" and 10 randomised test cases), so
the loader below normalises both into one internal representation:

    Shape A (dictionary style - used by the test cases):
        "warehouses": {"W1": [x, y], ...}
        "agents":     {"A1": [x, y], ...}
        "packages":   [{"id": "P1", "warehouse": "W1", "destination": [x, y]}]

    Shape B (list style - used by base_case.json):
        "warehouses": [{"id": "W1", "location": [x, y]}, ...]
        "agents":     [{"id": "A1", "location": [x, y]}, ...]
        "packages":   [{"id": "P1", "warehouse_id": "W1", "destination": [x, y]}]

Everything downstream (assignment, simulation, reporting) only ever works
with the normalised representation, so the rest of the logic does not
care which shape the file originally used.
"""
